  ...
  let interval = setInterval(() => {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        console.log("  → Status:", res.status || res.task_status);
...
async function downloadViaProxy(url, filename, textContent) {
  ...
  // 🔥 BƯỚC 2: NẾU LÀ AI84 & CÓ JOB_ID → REFRESH URL
  if (url.includes("cdn.ai84.pro") && jobId) {
    console.log("🔄 Refreshing AI84 URL for job:", jobId);
